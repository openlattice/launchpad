...
class IntegrationTables {
    companion object {
        ...
        const val CREATE_INTEGRATION_ACTIVITY_SQL = "CREATE TABLE IF NOT EXISTS integration_activity (" +
                    ...
                    "finish timestamptz DEFAULT 'infinity', " +
                    "result text, " +
                    "failureCause text DEFAULT '', " +
         ...
         * 4. start
         */
        const val LOG_SUCCESSFUL_INTEGRATION = "UPDATE integration_activity SET finish = now(), result = 'SUCCESS' WHERE integration_name = ? AND host_name = ? AND table_name = ? AND start = ? "
        ...
        const val LOG_FAILED_INTEGRATION = "UPDATE integration_activity " +
                "SET finish = now(), result = 'FAILED', cause = ? " +
